...
def create_google_search_graph(
    ...
    async def llm_search_node(state: WorkersState):
        ...
        if not state["workers_messages"] or state["workers_messages"][0].type != "system":
    ...
    worker_builder.add_node("search_tool_node", ToolNode(tools=tools, messages_key="workers_messages"))
