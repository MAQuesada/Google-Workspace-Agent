...
from oauth import generate_oauth_url, exchange_code_for_tokens, get_user_email_from_token, validate_oauth_code, generate_pkce_pair
from utils.config import get_config  # Use centralized config
...
class Config:
    def __init__(self):
        self.config = get_config()  # Use utils.config
        self.GOOGLE_CLIENT_ID = self.config.GOOGLE_CLIENT_ID
        self.GOOGLE_CLIENT_SECRET = self.config.GOOGLE_CLIENT_SECRET
        self.GOOGLE_TOKEN_URL = self.config.GOOGLE_TOKEN_URL
        self.API_BASE_URL = os.getenv("API_BASE_URL", "http://localhost:8000")
        self.GOOGLE_SCOPES = self.config.GOOGLE_SCOPES
...
def refresh_user_data():
    try:
        with st.spinner("Refreshing account status..."):
            ...
            st.session_state.accounts = data.get("accounts", [])
            logger.info(f"User data refreshed for {st.session_state.username}")
    except Exception as e:
...
class AuthCodeHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if code and not error:
            self.wfile.write(b"<html><body><h1>Authentication complete</h1><p>You can close this window and return to the app.</p></body></html>")
            ...
            self.wfile.write(b"<html><body><h1>Authentication failed</h1><p>An error occurred. Please close this window and try again.</p></body></html>")
...
def render_oauth_flow():
    if st.session_state.oauth_state == "awaiting_code":
        ...
        **Steps:**
        ...
        # Check for timeout
        if st.session_state.start_oauth_time and time.time() - st.session_state.start_oauth_time > UIConstants.OAUTH_TIMEOUT_SECONDS:
...
def process_oauth_code(code: str, account_label: str):
    ...
    try:
        ...
        if not tokens.get("refresh_token"):
            raise ValueError("No refresh token received. The account may have been previously authorized. Try revoking access in your Google Account settings and try again.")
        ...
        flow_type = st.session_state.get('oauth_flow_type', 'association')
        if flow_type == 'additional-association':
            st.success(f"Additional account `{user_email}` has been added successfully!")
        elif flow_type == 're-authentication':
            st.success(f"Account `{user_email}` has been re-authenticated successfully!")
        else:
            st.success(f"Account `{user_email}` has been associated successfully!")
        logger.info(f"OAuth association successful for {st.session_state.username}: {user_email}")
        time.sleep(2)
        reset_oauth_state()
        refresh_user_data()
        st.rerun()
        ...
        logger.error(f"OAuth processing failed for {st.session_state.username}: {str(e)}")
...
def start_oauth_flow(flow_type: str):
    try:
        with st.spinner("Preparing authentication..."):
            ...
            server, thread = start_local_server(port, on_code_received, state_lock, code_received_event)
        ...
        logger.info(f"OAuth flow started for {st.session_state.username} - {flow_type}")
        st.rerun()
    except Exception as e:
        show_error("Failed to start OAuth flow", str(e))
        logger.error(f"OAuth flow start failed: {str(e)}")
    ...
    if st.session_state.accounts:
        ...
        for i, account in enumerate(st.session_state.accounts):
            with st.container():
                ...
                with col2:
                    if is_expired:
                        if st.button(f"Re-auth", key=f"reauth_{i}", help=f"Re-authenticate {account_email}"):
                            with st.form(key=f"reauth_form_{i}"):
                                account_label = st.text_input("Account Label:", value=label)
                                submit = st.form_submit_button("Start Re-authentication")
                                if submit:
                                    try:
                                        validated_label = validate_account_label(account_label)
                                        ...
                                        st.error(str(e))
                    if st.button(f"Remove", key=f"remove_{i}", help=f"Remove {account_email}"):
                        try:
                            with st.spinner(f"Removing {account_email}..."):
                                st.session_state.api_client.dissociate_account(st.session_state.username, account_email)
                            st.success(f"Account {account_email} removed successfully.")
                            refresh_user_data()
                            st.rerun()
                        except Exception as e:
                            show_error("Failed to remove account", str(e))
                            logger.error(f"Account removal failed for {account_email}: {str(e)}")
        ...
        if working_accounts:
            ...
            account_options = [f"{acc.get('account_email')} ({acc.get('account_info').split(' (')[0] if '(' in acc.get('account_info', '') else acc.get('account_info')})" for acc in working_accounts]
            ...
            with col2:
                with st.form(key="add_account_form"):
                    account_label = st.text_input("Account Label:", placeholder="e.g., work, personal, project-name")
                    submit = st.form_submit_button("Associate Google Account", type="secondary")
                    if submit:
                        try:
                            validated_label = validate_account_label(account_label)
                            st.session_state.account_label = validated_label
            ...
            st.warning("⚠️ No accounts are currently active. Please associate or re-authenticate an account to access chat.")
            
    else:
        st.warning("No Google accounts associated with your profile.")
        st.info("You need to associate at least one Google account to access workspace features and chat.")
...
def debug_oauth_state():
    if st.sidebar.checkbox("Show Debug Info", value=False):
        ...
        st.sidebar.write(f"OAuth URL Present: {'Yes' if st.session_state.oauth_url else 'No'}")
        st.sidebar.write(f"Auth Code Present: {'Yes' if st.session_state.auth_code else 'No'}")
        st.sidebar.write(f"Flow Type: {st.session_state.get('oauth_flow_type', 'N/A')}")
        st.sidebar.write(f"Accounts count: {len(st.session_state.accounts) if st.session_state.accounts is not None else 'Not set'}")
...
def main():
    st.set_page_config(page_title="Google Workspace Multi-Agent Interface", page_icon="🔧", layout="wide")
