...
def main():
    ...
    if not working_accounts:
        st.error("No active Google accounts available. Please re-authenticate your accounts.")
        if st.button("Go to Account Management"):
            st.switch_page("main.py")
        return
    ...
    st.info(f"Using account: **{selected_account['account_email']}** ({account_labels[selected_account_idx].split('(')[1].rstrip(')')})")
    ...
    if "current_conversation_id" not in st.session_state or st.session_state.current_conversation_id != conversation_id:
    ...
    with st.sidebar:
        ...
        st.info(f"**Messages:** {len(st.session_state.chat_messages.get(conversation_id, []))}")
            ...
            try:
                ...
                if session_conversations:
                    ...
                    selected_conv_for_history = st.selectbox(
                        ...
                        index=session_conversations.index(conversation_id) if conversation_id in session_conversations else 0,
                        key="history_conv_select"
                    )
                    
                    # Load and show history for selected conversation
                    if st.button("Load History", use_container_width=True):
                        if selected_conv_for_history not in st.session_state.current_history:
                            try:
                                with st.spinner("Loading chat history..."):
                                    history = client.get_chat_history(username, selected_conv_for_history)
                                    st.session_state.current_history[selected_conv_for_history] = history or []
                                    st.success(f"Loaded {len(history or [])} messages")
                            except Exception as e:
                                st.error(f"Failed to load history: {str(e)}")
                                st.session_state.current_history[selected_conv_for_history] = []
                        else:
                    ...
                    if selected_conv_for_history in st.session_state.current_history:
                        if st.button("Clear Loaded History", use_container_width=True, type="secondary"):
                            if selected_conv_for_history in st.session_state.current_history:
                                del st.session_state.current_history[selected_conv_for_history]
    ...
    if st.session_state.show_history:
        selected_conv_for_history = st.session_state.get("history_conv_select", conversation_id)
        ...
        if selected_conv_for_history in st.session_state.current_history:
            ...
            with st.expander(f"Previous Messages ({len(history)} messages)", expanded=True):
                if history:
    ...
    with st.expander("Chat Tips & Information", expanded=False):
        account_label = account_labels[selected_account_idx].split('(')[1].rstrip(')')
        st.markdown(f"""
        **Current Session:**
        - **Conversation ID**: `{conversation_id}`
        - **Selected Account**: `{selected_account['account_email']}`
