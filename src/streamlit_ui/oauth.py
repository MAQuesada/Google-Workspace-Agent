...
def format_scopes_for_display(scopes: list) -> str:
    ...
    return "\n".join(descriptions)import base64
import hashlib
import os
import urllib.parse
import requests
...
def generate_oauth_url(
    config, redirect_uri: str, code_challenge: str
) -> Dict[str, str]:
    """Generate OAuth authorization URL with PKCE"""
    params = {
        "client_id": config.GOOGLE_CLIENT_ID,
...
def exchange_code_for_tokens(
    code: str, redirect_uri: str, code_verifier: str, config
) -> Dict[str, Any]:
    """
    Exchange authorization code for access and refresh tokens
    Replicates the token exchange logic from consent_flow.py
    ...
    try:
        ...
        # Make the token exchange request
        response = requests.post(config.GOOGLE_TOKEN_URL, data=token_data, timeout=30)
        ...
        if "refresh_token" not in tokens:
            logger.warning(
                "No refresh token received - this may indicate the user has already authorized this app"
            )
            raise Exception(
                "No refresh token received. Please revoke this app's permissions in your Google Account settings and try again."
            )
...
def get_user_email_from_token(access_token: str) -> str:
    ...
    try:
        ...
        logger.error("Network error while fetching user email", extra={"error": str(e)})
        raise Exception(f"Network error while fetching user email: {str(e)}")
    except Exception as e:
        logger.error("Failed to get user email", extra={"error": str(e)})
        raise
...
def validate_tokens(tokens: Dict[str, Any]) -> bool:
    ...
    for field in required_fields:
        if field not in tokens:
            logger.error(
                "Missing required field in token response", extra={"field": field}
            )
            return False
    ...
    if tokens.get("token_type", "").lower() != "bearer":
        logger.warning(
            "Unexpected token type", extra={"token_type": tokens.get("token_type")}
        )
...
def refresh_access_token(refresh_token: str, config) -> Dict[str, Any]:
    ...
    try:
        ...
        response = requests.post(config.GOOGLE_TOKEN_URL, data=refresh_data, timeout=30)
        ...
        logger.error("Network error during token refresh", extra={"error": str(e)})
        raise Exception(f"Network error during token refresh: {str(e)}")
    except Exception as e:
...
def test_token_validity(access_token: str) -> bool:
    ...
    try:
        ...
        logger.warning("Token validity test failed", extra={"error": str(e)})
        return False
...
def extract_code_from_url(url: str) -> str:
    ...
    try:
        ...
        logger.warning("Failed to extract code from URL", extra={"error": str(e)})
        return ""
